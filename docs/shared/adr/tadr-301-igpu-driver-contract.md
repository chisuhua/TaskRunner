...
6. `stream_id` parameter (across `submit_*` / `stream_capture_*` / `submit_graph`) is `uint32_t` in TaskRunner; upstream UsrLinuxEmu has widened to `uint64_t` with u32 backward-compat fallback. Known cross-repo drift — see [tadr-306 stream_id u32 ↔ u64 Cross-Repo Drift](./tadr-306-stream-id-u64-contract-align.md) (PROPOSED, exception to rule 1 until realignment trigger fires).
