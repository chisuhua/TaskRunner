...
STATUS: PROPOSED
DATE: 2026-07-11
CHANGE: g-gpu-client-meyers-singleton-fallback
RELATED: include/test_fixture/gpu_driver_client.h:836 (g_gpu_client declaration)
RELATED: src/test_fixture/gpu_driver_client.cpp:11 (g_gpu_client definition)
RELATED: src/test_fixture/cuda_stub.cpp (CudaStub implementation, no global instance)
SUPERSEDES: g-gpu-client-default-stub-init (v1, baseline was factually wrong)
RELATED_DRIFT: Oracle analysis bg_afcc8f4b — git log -S "CudaStub g_cuda_stub" returns only v1 spec commit; static instance never existed
---

# Capability: shim-default-init-fallback

> UMD shim functions (`cuStreamSynchronize`, `cuGraphLaunch`, `cuMemPoolCreate`)
> succeed when `g_gpu_client == nullptr` by falling back to a Meyers-singleton
> `CudaStub` instance, eliminating the need for explicit
> `g_gpu_client = &g_mock;` setup in shim consumer tests.
>
> **Status**: Spec-only (PROPOSED). Awaiting implementation.
>
> **Supersedes**: v1 `g-gpu-client-default-stub-init` (2026-07-09) — its baseline
> claim that `CudaStub g_cuda_stub;` exists as a static global instance in
> `src/test_fixture/cuda_stub.cpp` was factually wrong (verified via `git log -S
> "CudaStub g_cuda_stub"` returning only the v1 spec commit itself). v1 also
> failed to account for `init_gpu_client()` short-circuit behavior, which would
> have silently degraded CLI commands to mock behavior.
