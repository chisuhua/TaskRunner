...
**状态**: ✅ Partially Resolved (Issue #3 已修复，Issue #2/#1 待 H-3.7/H-3.8)
**日期**: 2026-06-23
...
**实施分工**:
- **TaskRunner 端 (本仓)**: 协调 + 文档 + 测试设计 (IN PROGRESS 2026-06-26)
- **UsrLinuxEmu 端 (外部仓)**: 实际代码修改 (TBD)

**跨仓协议**: 按 [ADR-035 §Rule 5.1](../../../../docs/00_adr/adr-035-governance-policy.md) 4 步流程，详见跨仓 PR 模板。

**预期时间表**:
- Day 1-3: TaskRunner 端协调文档 (IN PROGRESS 2026-06-26)
- Day 4-7: UsrLinuxEmu owner 评估 + 实施 PR 1 (TBD)
- Day 8-10: 跨仓 submodule bump + tadr-105 状态更新 (TBD)
- Day 11+: 可选 PR 2 (TBD)

**预期产出**:
- Issue #2 修复后，本 TADR §Issue #2 状态从 ⏸️ → ✅ Accepted
- 触发 H-3.8 (Issue #1 协调) 启动
...
- ⏸️ Issue #2 协调中（H-3.7 IN PROGRESS）
- ⏸️ Issue #1 待协调（H-3.8 后续）

### Deferred (H-3.8 触发时)

- ⏸️ Issue #1 修复需要 ABI 变更（u32 → u64 stream_id）— H-3.8 协调阶段
- ⏸️ Issue #2 修复需要新抽象层（GpuQueueEmu）— H-3.7 协调阶段 **IN PROGRESS**
...
**最后更新**: 2026-06-26（H-3.6 完成 §Issue #3 → Accepted，H-3.7 启动 §Issue #2 协调）
