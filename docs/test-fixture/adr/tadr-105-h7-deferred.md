...
**状态**: ✅ Fully Resolved (Issue #3/#2/#1 全部修复)
**日期**: 2026-06-26
...
|-------|------|----------------------|
| #1 stream_id u32 ↔ queue_handle u64 类型不匹配 | `next_queue_handle_` 超过 `UINT32_MAX` 时 create_queue 失败 | **✅ 已修复** (02ae421: `gpu_ioctl.h:43` stream_id u32→u64 + `stream_id_compat` deprecated alias + backward compat fallback) |
...
### Trigger Conditions（历史记录 — 3 issues 均已修复）

以下 trigger conditions 是 H-3.6/H-3.7/H-3.8 协调阶段的启动条件，所有 3 个 issues 均已修复：

1. `next_queue_handle_` 接近 `UINT32_MAX` (Issue #1 触发) — **✅ 已修复** (02ae421: u32→u64 拓宽)
2. 生产环境首次 `submit_batch` 触发 `-EINVAL` 且 root cause 是 stream_id 截断 (Issue #1 触发) — **✅ 已修复**
3. mmap 快速路径 (`MAP_QUEUE_RING`) 被实施 (Issue #2 触发) — **✅ 已修复** (392a496: GpuQueueEmu 抽象层)
4. 第三方 GPU service 接入 UsrLinuxEmu — 条件未触发
5. H-3.6 协调启动 (2026-06-26 主动触发，Issue #3 修复协调) — **✅ 已修复** (bf8192f + 09ae1b0)
...
**实际修复** (commit 02ae421):
- `gpu_ioctl.h:43`: ABI 字段 `stream_id` 从 `__u32` 拓宽为 `__u64`
- `gpu_ioctl.h:43-44`: 新增 `__u32 stream_id_compat` (deprecated alias) + `__u32 flags_extended` (reserved)
- `gpgpu_device.cpp:262`: 移除 `static_cast<uint64_t>(args->stream_id)` R2 mapping 截断
- `gpgpu_device.cpp:262-268`: 增加 backward compat fallback 逻辑（`stream_id==0 && stream_id_compat!=0` 自动回退）
- 错误日志使用 `effective_stream_id`（不再截断）
- 6 月过渡期（2026-06-26 ~ 2026-12-26）：旧调用方通过 `stream_id_compat` 继续工作

**测试验证**:
- 编译通过：`cd UsrLinuxEmu/build && make -j4` (0 error)
- 现有测试全通过：`test_gpu_plugin` + `test_gpu_fence_return` + `test_queue_puller_integration`
- u64 边界值测试：`next_queue_handle_` > `UINT32_MAX` 正常创建/提交
- ABI 向后兼容测试：旧 caller 通过 `stream_id_compat` 正常工作，新 caller 通过 `stream_id` (u64) 正常工作

**验证状态**: ✅ 功能修复完成 (H-3.8 完成，H-7 deferred 3 issues 全部解决)

**跨仓协议**: 按 [ADR-035 §Rule 5.1](../../../../docs/00_adr/adr-035-governance-policy.md) 4 步流程，详见跨仓 PR 模板。

**预期时间表**:
- Day 1-3: TaskRunner 端协调文档 (DONE 2026-06-26)
- Day 4-7: UsrLinuxEmu owner 评估 + 实施 PR 1 (DONE 2026-06-26, 02ae421)
- Day 8-10: 跨仓 submodule bump + tadr-105 状态更新 (DONE 2026-06-26)

**预期产出**:
- ✅ Issue #1 修复完成，状态从 ⏸️ → ✅ Accepted
- ✅ H-7 deferred 3 issues 全部解决
- ✅ 生产硬上限消除（u64 handle 支持，从 ~40 亿提升到 2^64）
- ✅ H-7 tracking 全部关闭
...
### Current (H-3.8 已完成，H-7 deferred 3 issues 全部 shippable)
...
- ✅ Issue #1 已修复（02ae421: stream_id u32→u64 ABI 拓宽 + deprecated alias）
- ✅ H-7 deferred 3 issues 全部解决

### Resolved (H-3.6/H-3.7/H-3.8 全部完成)

- ✅ Issue #1 已修复（02ae421: stream_id u32→u64 + deprecated alias + backward compat fallback）
- ✅ Issue #2 已修复（GpuQueueEmu 抽象层委托，392a496）
- ✅ Issue #3 已修复（PUSHBUFFER_SUBMIT_BATCH 强校验 + 4 cases 测试，bf8192f + 09ae1b0）

### Mitigation (不适用)

- 所有 3 个 H-7 issues 均已修复，无需 mitigation
- 📚 **注意**: `stream_id_compat` deprecated alias 将在 2026-12-26 后通过 PR 2 移除。过渡期内旧调用方仍可使用该字段。
- 📚 **注意**: 生产硬上限从 ~40 亿（u32）提升到 2^64（u64），理论无限
...
**最后更新**: 2026-06-26（H-3.6 §Issue #3 → Accepted，H-3.7 §Issue #2 → Accepted，H-3.8 §Issue #1 → Accepted，H-7 deferred 3 issues 全部关闭）
