...
STATUS: PROPOSED
DATE: 2026-07-09
CHANGE: test-cu-graph-coverage-fixes
RELATED: tadr-301-igpu-driver-contract.md (reuse wait_fence overload)
RELATED_UPSTREAM: chisuhua/UsrLinuxEmu phase4-sim-graph-launch-real-impl (merged 2026-07-09)
RELATED: plans/sync-plan.md (drift fix: remove false test_cu_graph_real, recount Total)
---

# Capability: cu-graph-async-fence-testing

> End-to-end testing of `cuGraphLaunch` + `cuStreamSynchronize` async fence
> lifecycle in the TaskRunner UMD shim. Closes 3 critical test gaps that
> appeared after UsrLinuxEmu's `phase4-sim-graph-launch-real-impl` made
> `cuGraphLaunch` truly async (fences now signaled by
> `HardwarePullerEmu::handleComplete()` after batch completion, not
> immediately).
>
> **Status**: Code implementation complete; awaiting final ctest + openspec
> validate verification before archive.
...
None for this delta. The change reuses the existing
`IGpuDriver::wait_fence(uint64_t, uint32_t, uint32_t*)` overload at
`include/shared/igpu_driver.hpp:228` and does NOT modify any existing
specification requirements. The Phase 4 base requirements live in
`openspec/changes/archive/2026-07-07-phase3-real-impl-bridge/`.

## REMOVED Requirements

None. All existing test coverage is preserved (30 pre-existing cases still
pass; the 2 new cases add coverage). No capability is retired.
...
- **IGpuDriver interface changes** — no new virtual methods, no vtable
  expansion. The existing `wait_fence` overloads (3-arg and 1-arg) are
  reused as-is.
- **`include/shared/` modifications** — none. The `shared` scope is NOT
  touched; dual review per AGENTS.md does NOT apply.
- **`cmd_cuda.cpp` / CLI `cuda_graph_*` commands** — deferred to a future
  change; this one only closes test coverage gaps.
- **Multi-fence tracking per stream** — only the most-recent fence is
  tracked. Multi-launch scenarios are deferred to Phase 5.
- **UsrLinuxEmu production code** — no changes; this change is TaskRunner-only
  (notification-only cross-repo coordination).